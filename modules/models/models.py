                ...
                if "/imagine" in mj_prompt:
                    ...
                    if mj_req.status_code == 200:
                        ...
                        if mj_status[0]["status"] != "FAILURE":
                            ...
                            client = poe.Client(token)
                            poe.logger.setLevel(logging.INFO)
                            message = "display the image url first, then render it into markdown:"+image_url
                            response = client.send_message(model, message, with_chat_break=True)
                            print(response)
                        else:
                            token = poe_token
                    ...
                    if upscale_flag == True:
                        ...
                        if upscale_req.status_code == 200:
                            ...
                            if mj_status2[0]["status"] != "FAILURE":
                                ...
                                client = poe.Client(token)
                                poe.logger.setLevel(logging.INFO)
                                message = "display the image url first, then render it into markdown:"+upscale_url
                                response = client.send_message(model, message, with_chat_break=True)
                                print(response)
                            else:
                                token = poe_token
                    ...
                    if v_flag == True:
                        ...
                        if variation_req.status_code == 200:
                            ...
                            if mj_status3[0]["status"] != "FAILURE":
                                ...
                                client = poe.Client(token)
                                poe.logger.setLevel(logging.INFO)
                                message = "display the image url first, then render it into markdown:"+variation_url
                                response = client.send_message(model, message, with_chat_break=True)
                                print(response)
                            else:
                                token = poe_token
